...
class APIConnectionError(APIError):
    ...
    This typically indicates issues with the client's network, internet service
    provider, or intermediate network infrastructure rather than problems with
...
class APIServerError(APIError):
    ...
    Implementing retry logic with exponential backoff is recommended for
...
class APIClientError(APIError):
    ...
    This exception occurs when there are errors related to the client request
    (status codes 400-499), such as:
    - 400 Bad Request: The request was malformed or contained invalid
      parameters
    - 403 Forbidden: The client lacks necessary permissions for the requested
      resource
